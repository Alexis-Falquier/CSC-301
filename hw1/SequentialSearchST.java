...
public class SequentialSearchST<Key extends Comparable<Key>, Value> {
    ...
    public Value get(Key key) {
    	if (key == null) throw new IllegalArgumentException("Key to get cannot be null");
    	Node x = first;
    ...
    public void put(Key key, Value val) {
    	...
    	Node x = first;
    	Node pNode = new Node(key, val, null);
    	if (n == 0){
    		first = pNode;
    		n++;
    		return;
    ...
    public void delete(Key key) {
    	if (key == null) throw new IllegalArgumentException("Key to delete cannot be null");
    	if (n == 0) return;
    	Node x = first;
    	if (x.key.equals(key)){
    		first = x.next;
